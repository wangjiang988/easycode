...
const { ask, askIfContinue, muilpulAsk, groupAsk, askAQuestion} = require('./ask')
const { templateInfo, generateYamlFile, arryObject_to_object, cloneGit, cloneGitPromise, getGitName, deleteFolder } = require('./common')
...
class EasyCode {
  ...
  async init(callback) {
    ...
    answers.tplGit ={
      path: answers.git_path,
      branch: answers.git_branch
    }
    delete answers.git_branch
    delete answers.git_path
    ...
    // 询问是否设置全局变量
    answers.metadata = {
      "replacement_for_tpl": "hi easycode"
    }
    ...
    if (answers.tplGit.path !== '') {
      const tplPath = getGitName(answers.tplGit.path)
      cloneGit(answers.tplGit.path, answers.tplGit.branch, 'templates/'+tplPath)
    }
  ...
  makeSingleTemplate(templateName) {
    ...
    // 生成模板
    this.domakeTemplate(targetTemplate, this)
  }
   ...
   */
  domakeTemplate(targetTemplate, easycode) {
    TemplateParser.parse(targetTemplate.filePath, easycode, (t) => {
  ...
  async AsyncDomakeTemplate(targetTemplate, useDefault) {
    ...
    await this.generator.generateSingle(useDefault)
  }
  ...
  async makeGroupTemplate(groupName) {
    ...
    const useDefault = await askAQuestion('是否使用默认值生成文件', 'Y')
    // 开始生成group模板组
    if (targetGroup !== null && targetGroup.children.length > 0) {
      for (let j = 0; j < targetGroup.children.length; j++) {
        ...
        await this.AsyncDomakeTemplate(singleTemplate, useDefault)
      }
      logger.success('文件组生成结束!')
    }
  }
  ...
  async reloadTemplateConfig () {
    ...
    // 要先删除对应的git库代码
    try {
      if (config.tplGit.path !== '') {
        const tplPath = getGitName(config.tplGit.path)
        // 先删除原文件夹
        const targetPath = path.resolve('templates/'+ tplPath)
        deleteFolder(targetPath)
        // 下载新git代码
        let ret = await cloneGitPromise(config.tplGit.path, config.tplGit.branch, 'templates/'+ tplPath)
        console.log(ret)
      }
