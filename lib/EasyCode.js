...
const { ask, muilpulAsk } = require('./ask')
const { templateInfo, generateYamlFile, arryObject_to_object } = require('./common')

// TemplateParser.parse('demo_single_template.tpl', (t) => {
...
class EasyCode {
  ...
  async scanTemplatePath () {
    ...
    await templateInfo(templatePath, templatePath, templateInfos)
  ...
  async AsyncDomakeTemplate(targetTemplate) {
    const t = await TemplateParser.AsyncParse(targetTemplate.filePath)
    this.generator.init(t)
