...
const { ask, askAQuestion, muilpulAsk, getDefaultAskVariabaleMap } = require('./ask')
...
module.exports = class Generator {
  ...
  generate(useDefault) {
    if (this.template_type === 'single') {
      this.generateSingle(useDefault)
    }
  }

  async generateSingle(useDefault) {
     // 从easycode对象中获取全局变量
    const globalVaris = this.getGlobalVariablesFromEasycode()
    // 根据变量 解析字符串
    this.template.filename = parseVaris(globalVaris, this.template.filename)

    const metalsmith = MetalSmith(this.template.templateRootPath)

    // 是否询问 如果使用默认值 则不要询问
    let shouldAsk = true
    if (useDefault == 'Y' || useDefault == 'y' || useDefault == 'yes') {
      shouldAsk = false
    }

    // 单文件询问生成文件名称

    let targetFileNameAns = {
      targetFileName: this.template.filename
    }
    let metadata = getDefaultAskVariabaleMap(this.template.variables)
    if (shouldAsk) {
      targetFileNameAns = await ask([{
        name: 'targetFileName',
        message: '待生成文件名称',
        default: this.template.filename
      }])
      metadata = await ask(this.template.variables)
    }
    ...
    console.log(chalk.yellow('   开始生成文件:' + metadata.targetFileName))
    ...
    let targetPath = defaultPositiveTargetPath
    if (shouldAsk) {
      targetPath = await askAQuestion('文件生成地址:(相对于当前路径)', defaultPositiveTargetPath)
    }

    targetPath = getProjectAbosutePath(targetPath)

    console.log(chalk.yellow('   文件生成路径:' + targetPath))
